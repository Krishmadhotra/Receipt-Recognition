...
function App() {
  ...
  const formatResult = (result) => {
    ...
    if (result.duplicates.length === 0) {
      ...
      // formattedData += `Score: ${duplicate.score.toFixed(4)}\n\n`
    } else {
      formattedData += 'Duplicates Found:\n';
      result.duplicates.forEach((duplicate, index) => {
        formattedData += `Pair ${index + 1}:\n`;
        formattedData += `\t\t File 1 \n`
        formattedData += `\t\t File 2 \n\n`;
        formattedData += `\tScore: ${duplicate.score.toFixed(4)}\n\n`;
      });
    }

    formattedData += 'Files Processed:\n';
    result.files.forEach((file, index) => {
      formattedData += `File ${index + 1}: ${file}\n`;
    });
